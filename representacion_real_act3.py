...
# evaluar cuán buena es una distribución
...
# crea un hijo combinando genes de ambos padres
...
def mutacion_gaussiana(cromosoma, sigma=0.1):
    ...
    for i in range(39):
        ...
        genes_mutados = [max(0.0, g + random.gauss(0, sigma)) for g in genes]
        suma = sum(genes_mutados)
        if suma > 0:
            genes_normalizados = [g / suma for g in genes_mutados]
        else:
            genes_normalizados = [1/3, 1/3, 1/3] # fallback seguro???
